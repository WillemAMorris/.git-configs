...
case $desktop in
    ...
    # fi
    ;;
    awesomewm|/usr/share/xsessions/awesome)
    if type "xrandr" > /dev/null; then
      for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
        MONITOR=$m polybar --reload mainbar-awesomewm -c ~/.config/polybar/config.ini &
      done
    else
    polybar --reload mainbar-awesomewm -c ~/.config/polybar/config.ini &
    fi
    ;;
esac
